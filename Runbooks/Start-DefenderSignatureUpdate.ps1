...
$RunbookName = "Start-DefenderQuickScan"
...
try
{
    ...
    # Logic goes here
    ##
    $trace += "$([DateTime]::Now.ToString())`tStarting a QuickScan on server: $($ServerName)`n"
    ##
    
    Invoke-Command -ComputerName $ServerName -ScriptBlock {import-module Defender; Start-MpScan -ScanType QuickScan}
    
    $trace += "$([DateTime]::Now.ToString())`tFinished a QuickScan on server: $($ServerName)`n"
