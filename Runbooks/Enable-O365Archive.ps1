...
try
{
    ...
    # Logic - Get migrated mailboxes and enable remove archive
    $mb = Get-Mailbox -ResultSize Unlimited -Filter {ArchiveGuid -eq "00000000-0000-0000-0000-000000000000" -AND (Name -NotLike "DiscoverySearchMailbox*")}
    ...
    if ($mb.count -gt 0)
    {
        $mb | foreach {
            try
            {
                $usr = Get-Recipient -Identity $_.UserPrincipalName | ? {$_.AuthenticationType -eq "Managed"}

                if($usr -ne $null)
                {
                    $usr | Enable-Mailbox -Archive -Confirm:$false
