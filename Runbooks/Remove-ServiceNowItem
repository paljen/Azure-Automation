...
$RunbookName = "Remove-ServiceNowItem"

try
{
    <# Data used for testing and debugging, adjust to fit needs
    #$sysid = "ec12655ddbd9760096e8f7461d9619da"
    $table = "u_azure_automation_runbook"
    #$item = @{'u_runbook_id' = 100}
    #>
    ...
    $response = $null
    $response = Invoke-WebRequest -Method $method -Uri $uri -ContentType $contentType -Credential $Cred -UseBasicParsing
    ...
    $rbLog.WriteLogEntry($RunbookName,"Removed Record with Sys_Id $sysid from table $table, StatusCode: $($response.statusCode)")
