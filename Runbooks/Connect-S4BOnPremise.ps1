...
<#
.DESCRIPTION
    Connect to skype for business on-premise
...
$RunbookName = "Connect-S4BOnOnPremise"
...
try
{
    ...
    $uri = Get-AutomationVariable -Name "ConnectionURI-SkypeOnPremises"

    $session = New-PSSession -Name $RunbookName -ConnectionUri $uri -Credential $cred
    ...
    if($session.Availability -eq "Available" -and $module -ne $null){
        ...
        Throw "Failed importing module $($module.Name) from $($session.ComputerName)"
    }
