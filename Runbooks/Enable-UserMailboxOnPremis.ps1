...
<#
.DESCRIPTION
    Creates a user mailbox on premesis
...
.NOTES
    ...
    Note     : 1.0 - 14-09-2017, Admin-PJE, Initial runbook development
#>
...
$RunbookName = "Enable-OnPremiseUserMailbox"
...
try
{
    ...
    $mb = Enable-Mailbox -Identity $UserPrincipalName -PrimarySmtpAddress $PrimarySmtpAddress -DomainController "dkhqdc02.prd.eccocorp.net"

    Set-Mailbox $userPrincipalName -EmailAddressPolicyEnabled $true -Force -DomainController "dkhqdc02.prd.eccocorp.net"
    
    $rbLog.WriteLogEntry($RunbookName,"Successfully enabled mailbox for user $UserPrincipalName")

    if($Archive){
        Enable-Mailbox -Identity $UserPrincipalName -DomainController "dkhqdc02.prd.eccocorp.net" -Archive | Out-Null
        $rbLog.WriteLogEntry($RunbookName,"Successfully enabled archive for user $UserPrincipalName")

        Set-Mailbox $userPrincipalName -RetentionPolicy "Ecco Default Retention Policy" -DomainController "dkhqdc02.prd.eccocorp.net" | Out-Null
        $rbLog.WriteLogEntry($RunbookName,"Successfully set retention policy on mailbox $UserPrincipalName")
    }
